  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (metadata.kind === 'package' && metadata.passportId && metadata.packageType) {
      const updated = await db.passport.update({
        ...
        select: { id: true, userId: true, autoShareLandlordId: true, autoShareLandlordEmail: true }
      });

      if (updated.autoShareLandlordEmail) {
        const existingShare = await db.share.findFirst({
          where: { passportId: updated.id, landlordEmail: updated.autoShareLandlordEmail }
        });
        if (!existingShare) {
          ...
          await db.share.create({
            data: {
              ...
              landlordEmail: updated.autoShareLandlordEmail,
              landlordId: updated.autoShareLandlordId || null,
              feePaid: true
            }
          });
        }
